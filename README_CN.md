...
项目转为尽力维护模式后的第一个版本，重点是安全加固和已报告的 bug 修复。

- **Web 面板与 manager 服务安全加固**：可选 manager 共享密钥、安全响应头、错误信息脱敏、按 IP 的 API 限流、备份下载改用短时签名 token。
- **指标端口默认不再对外暴露**：Prometheus 端口默认绑定 `127.0.0.1`，需要远程抓取时设置 `METRICS_BIND_HOST=0.0.0.0`。
- **SMAPI 升级到 4.5.2**（与随附的游戏 1.6.15 兼容）。
- **修复状态报告器在镜像缺少 `nc`（netcat）时约 25% 的 CPU 忙循环**（#4）。
